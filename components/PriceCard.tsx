...
export function PriceCard({ label, price, isMain }: PriceCardProps) {
  ...
  return (
    <View className={`rounded-2xl p-6 border ${
      ...
      <Text allowFontScaling={false} className={`font-body text-sm mb-2 uppercase tracking-widest ${
        isMain ? 'text-accent font-bold' : 'text-text-secondary'
      }`}>
        {label}
      </Text>
      <View className="flex-row items-baseline">
        <Text allowFontScaling={false} className={`font-display font-bold ${
          isMain ? 'text-text-primary text-4xl' : 'text-text-primary text-2xl'
        }`}>
          {formattedPrice}
        </Text>
        <Text allowFontScaling={false} className={`font-body ml-1 ${
          isMain ? 'text-text-primary text-lg' : 'text-text-secondary text-sm'
        }`}>
          DZD
        </Text>
      </View>
      <Text allowFontScaling={false} className={`font-body mt-1 ${isMain ? 'text-gray-400 text-sm' : 'text-gray-400 text-xs'}`}>
        (soit ~ {Math.round(price / 10000).toLocaleString('fr-DZ')} Millions)
      </Text>
    </View>
  );
}
